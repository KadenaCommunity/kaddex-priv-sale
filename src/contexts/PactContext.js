...
export const PactProvider = (props) => {
        ...
        let cmd = {
          ...
          envData: {
            ...
            amountMinimum0: reduceBalance(
              amountDesired0 * (1 - parseFloat(slippage)),
              tokenData[token0.name].precision
            ),
            amountMinimum1: reduceBalance(
              amountDesired1 * (1 - parseFloat(slippage)),
              tokenData[token1.name].precision
            ),
          },
          meta: Pact.lang.mkMeta(
  ...
  const getPairAccountBalance = async (token0, token1, account) => {
    try {
      let data = await Pact.fetch.local(
        {
          pactCode: `(kswap.tokens.get-balance (kswap.exchange.get-pair-key ${token0} ${token1}) ${JSON.stringify(
            account
          )})`,
          meta: Pact.lang.mkMeta(
            "",
      ...
      let data = await Pact.fetch.local(
        {
          ...
          (let*
            (
              ...
              (reserveB (reserve-for p ${token1}))
              (totalBal (kswap.tokens.total-supply (kswap.exchange.get-pair-key ${token0} ${token1})))
              (acctBal (kswap.tokens.get-balance (kswap.exchange.get-pair-key ${token0} ${token1}) ${JSON.stringify(
            account
          )}))
  ...
  const swapLocal = async (token0, token1, isSwapIn) => {
    ...
    console.log("account.guard.keys[0]", account.guard.keys[0]);
    try {
      let privKey = signing.key;
      if (signing.method === "pk+pw") {
        const pw = await pwPrompt();
  ...
  const getReservations = async () => {
    try {
      ...
      if (data.result.status === "success") {
        setReservations(data.result.data);
      }
    } catch (e) {
      setReservations([])
      console.log(e);
  ...
  const kpennyReserveLocal = async (amtKda) => {
    try {
      ...
      const ct = creationTime();
      const pactCode = `(kswap.kpenny.reserve ${JSON.stringify(
        account.account
      )} (read-decimal 'amt))`;
      const cmd = {
        pactCode: pactCode,
        keyPairs: {
          ...
          clist: [
            ...
            {
              name: `coin.TRANSFER`,
              args: [account.account, "kpenny-bank", reduceBalance(amtKda, 12)],
            },
          ],
        },
        envData: {
      ...
      const signCmd = {
        ...
        caps: [
          ...
          Pact.lang.mkCap(
            ...
            "trasnsfer token in",
            `coin.TRANSFER`,
            [account.account, "kpenny-bank", reduceBalance(amtKda, 12)]
          ),
        ],
        sender: "kswap-free-gas",
        gasLimit: 3000,
  ...
  const kpennyRedeemLocal = async () => {
    try {
      ...
      const ct = creationTime();
      const pactCode = `(kswap.kpenny.redeem ${JSON.stringify(
        account.account
      )} ${JSON.stringify(account.account)} (read-keyset "user-ks"))`;
      const cmd = {
        pactCode: pactCode,
        keyPairs: {
          publicKey: account.guard.keys[0],
